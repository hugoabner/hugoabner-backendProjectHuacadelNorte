...
export const createProyect = async (req, res) => {
	try {
		const {
			name, category, description, imgURL, 
			estado
		} = req.body 
...
/**@funcion para obtener proyecto por el id */
export const getProyectById = async (req, res) => {
	try {
		console.log(req.params.proyectId);
		const product = await Proyect.findById(
...
/**@funcion para actualizar proyecto por id */ 
export const updateProyectById = async	 (req, res) => {
	try {
		const updateProyect = await Proyect.findByIdAndUpdate(
			req.params.proyectId, req.body, {new: true}
		);
		res.status(201).json ({
...
/**@funcion para eliminar proyecto por id */
